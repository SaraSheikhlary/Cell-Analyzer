...
Updated with spatial hole detection for precise vacuolization analysis.
"""
...
class AnalysisParams:
    ...
    min_cell_area: int = 200  # Lowered to 200 to catch smaller platelets
    min_nucleus_area: int = 65
    nucleus_dark_percentile: float = 26.0
    vacuole_threshold_offset: float = 0.15  # How much brighter than the median a hole must be
...
def _segment_cells(gray: np.ndarray, params: AnalysisParams) -> np.ndarray:
    ...
    # Slightly relaxed thresholding
    thresh = filters.threshold_otsu(blurred)
    mask = blurred < (thresh + 0.02) 

    mask = morphology.remove_small_objects(mask, min_size=params.min_cell_area // 3)
    mask = morphology.remove_small_holes(mask, area_threshold=200)
    mask = morphology.closing(mask, morphology.disk(2))

    distance = ndi.distance_transform_edt(mask)
    coords = feature.peak_local_max(distance, min_distance=10, labels=mask)
...
def segment_and_analyze(
    ...
    global_vacuole_mask = np.zeros_like(cell_mask, dtype=bool)
    ...
    for creg in cell_regions:
        ...
        # --- TRUE SPATIAL VACUOLE DETECTION ---
        cell_sub = gray[minr:maxr, minc:maxc]
        cell_pixels = cell_sub[cell_bbox_mask]
        vacuolization_pct = 0.0
        if len(cell_pixels) > 0:
            median_val = np.median(cell_pixels)
            # Find holes that are significantly brighter than the cell body
            vac_mask_sub = (cell_sub > (median_val + params.vacuole_threshold_offset)) & cell_bbox_mask
            vac_mask_sub = morphology.remove_small_objects(vac_mask_sub, min_size=3)
            
            # Store in global mask for overlay drawing
            global_vacuole_mask[minr:maxr, minc:maxc] |= vac_mask_sub
            
            vac_area = float(np.sum(vac_mask_sub))
    ...
    return {
        ...
        "vacuole_mask": global_vacuole_mask, # Passed to overlay
        "overlay": _create_overlay(image, cell_mask, nucleus_mask, global_vacuole_mask),
        "summary": _calculate_summary(cells, was_inverted),
        "params_used": params,
    }


def _calculate_summary(cells, was_inverted):
    if not cells:
        return {"num_cells": 0, "num_abnormal": 0, "abnormal_pct": 0.0, "mean_nc_ratio": 0.0, "max_nc_ratio": 0.0, "image_inverted": was_inverted}
    nc_vals = [c["nc_ratio"] for c in cells]
    abnormal = sum(1 for c in cells if "Abnormal" in c["classification"])
    return {
        "num_cells": len(cells),
        "num_abnormal": abnormal,
        "abnormal_pct": round(100.0 * abnormal / len(cells), 1),
        "mean_nc_ratio": round(float(np.mean(nc_vals)), 3),
        "max_nc_ratio": round(float(np.max(nc_vals)), 3),
